...
_ACCESS_TOKEN_LIFETIME_MINUTES = 20
...
def create_access_token(payload: dict):
    expiration_time = datetime.datetime.now() + datetime.timedelta(
        seconds=_ACCESS_TOKEN_LIFETIME_MINUTES,
    )

    return jwt.encode(
        dict(**payload, exp=expiration_time.timestamp()),
...
def verify_access_token_and_get_payload(access_token: str):
    payload = jwt.decode(
        access_token, key=_ACCESS_JWT_SECRET_KEY, algorithms=[_JWT_ALGORITHM]
    )
