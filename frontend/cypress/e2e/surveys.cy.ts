...
describe('surveys', () => {
    it('create survey', () => {
        ...
        cy.intercept(Cypress.env('FRONTEND_URL') + '/_nuxt/*/**').as(
            'hydration',
        )
        cy.wait('@hydration')

        cy.get('input[placeholder="Enter the survey title"]').type(
            'test survey',
            {
                delay: 300,
                release: false,
        ...
        cy.get('form ul[role="listbox"]')
            .should('be.visible')
            .contains('Short text')
            .click()

        cy.get('input[placeholder="Enter the question text"]').type(
            'test question 1',
        )
        ...
        cy.get('form ul[role="listbox"]')
            .should('be.visible')
            .contains('Single choice')
            .click()

        cy.get('input[placeholder="Enter the question text')
            .eq(1)
